...
export async function POST(request) {
  ...
  try {
    ...
    const fileUri = formData.get('fileUri'); // Nueva opción para bypass
    ...
    if (!file && !fileUri) {
      console.warn('Advertencia: No se proporcionó archivo ni URI.');
      return NextResponse.json({ error: 'No se proporcionó ningún archivo de audio ni URI.' }, { status: 400 });
    }
    ...
    if (fileUri) {
      ...
      result = await processAudioForMinuta(buffer, file.type, file.name, {
        title,
        date,
        department,
        participants
